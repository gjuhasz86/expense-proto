...
<div class="card card-block">
  <table class="table table-sm table-hover table-responsive">
    ...
    <tbody>
      <tr *ngFor="let item of averages$|async">
        <td><span>{{item.category.shortName}}</span></td>
        <td><span>{{item.avg}}</span></td>
      </tr>
    </tbody>
  </table>
</div>
...
export class MonthlyChartComponent {
    ...
    private averages$ = Observable.combineLatest(this.monthlyStats$, this.categories$, this.startDate$, this.endDate$)
                                  .map(tup => this.calculateAverages(tup[0], tup[1], tup[2], tup[3]));
    ...
    calculateAverages(stats: StatItem[], categories: Category[], startDate: Date, endDate: Date): any {
        let res = categories.map(c => {
            let monthlyStats = this.collectStats(c, stats, startDate, endDate);
            let sum = _.chain(monthlyStats).reduce((acc, e) => acc + e[1], 0).value();
            let count = this.generateLabels(startDate, endDate).length;
            let avg = Math.floor(sum / count);
            return {category: c, avg: avg};
        });
        return _.chain(res).sortBy(x => x.avg * -1).value();
    }
    ...
    collectStats(category: Category, stats: StatItem[], startDate: Date, endDate: Date): MonthlyStatData[] {
